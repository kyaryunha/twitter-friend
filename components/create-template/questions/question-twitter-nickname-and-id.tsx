...
const QuestionTwitterNicknameAndId: FC<QuestionsStoreTypes>  = observer(({store}) => {
        return (
            <>
                ...
                <StyledLabel>
                    트위터 닉네임
                    <StyledInputText type="text" placeholder="ex. 까룬" value={store.twitterNickname} onChange={(e) =>(store.updateTwitterNickname(e.target.value))} />
                </StyledLabel>
                <StyledLabel>
                    트위터 ID
                    <StyledInputText type="text" placeholder="ex. shinhyun_main" value={store.twitterId} onChange={(e) =>(store.updateTwitterId(e.target.value))} />
                </StyledLabel>
            </>
        )
    }
